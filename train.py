...
def create_callbacks(training_model, prediction_model, validation_generator, args):
    ...
    if args.evaluation and validation_generator:
        if args.dataset_type == "coco":
            ...
            evaluation = Evaluate(
                validation_generator, prediction_model, tensorboard=tensorboard_callback
            )
        else:
    ...
    if parsed_args.gpu and parsed_args.batch_size < len(parsed_args.gpu.split(",")):
        raise ValueError(
            "Batch size ({}) must be equal to or higher than the number of GPUs ({})".format(
                parsed_args.batch_size, len(parsed_args.gpu.split(","))
            )
        )
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(
        description="Simple training script for training a EfficientDet network."
    )
    ...
    csv_parser.add_argument(
        "annotations_path", help="Path to CSV file containing annotations for training."
    )
    csv_parser.add_argument(
        "classes_path", help="Path to a CSV file containing class label mapping."
    )
    csv_parser.add_argument(
    ...
    parser.add_argument(
        "--weighted-bifpn", help="Use weighted BiFPN", action="store_true"
    )
...
def main(args=None):
    ...
    if args.gpu and len(args.gpu.split(",")) > 1:
        model = keras.utils.multi_gpu_model(
            model, gpus=list(map(int, args.gpu.split(",")))
        )
    model.summary()
