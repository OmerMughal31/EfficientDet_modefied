...
def _get_detections(
    generator, model, score_threshold=0.05, max_detections=100, visualize=False
):
    """
    ...
    for i in progressbar.progressbar(
        range(generator.size()), prefix="Running network: "
    ):
...
def _get_annotations(generator):
    ...
    for i in progressbar.progressbar(
        range(generator.size()), prefix="Parsing annotations: "
    ):
...
if __name__ == "__main__":
    ...
    """
    Usage:
        Please add following values to repespective variables
        1- "path_test_csv" = it is the annotation file in CSV formate for test set
        2- "path_classes_csv" = path to the file that contains all names and corresponding labels for the classes
        3- "path_base_dir" = base directory path, where this eveluation file is present
        4- "checkpoints" is the base directory for model weight matrix. While, models are saved in sub-directoryies. So, give the path of your model dierectory as

        "2020-07-26 (B1 MND WBiFPN Perfect)"

        5- Give your .h5 (ckp) name in directory as
        "csv_300_0.0398_0.6976.h5"

        MOST IMPORTANTS:------------------> Set EfficientDet arguments for
                                            i- phi (to specify that which EfficientDet model are you using)
                                            ii- weighted_bifpb (if you have used the weighted feature fusion)
                                            iii- quad_angle (if you have quad annotations)

        6- pleased do not change batch size
    ...
    path_test_csv = "datasets/config_folder/dataset/test.csv"
    path_classes_csv = "datasets/config_folder/dataset/classes.csv"
    paths_base_dir = "/workspaces/EfficientDet_arranged/"

    # Please add the model path here
    ckp_base_path = "checkpoints/"
    ...
    model, prediction_model = efficientdet(
        phi=phi, num_classes=num_classes, weighted_bifpn=weighted_bifpn
    )
