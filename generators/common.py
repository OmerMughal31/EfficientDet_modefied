...
class Generator(keras.utils.Sequence):
    ...
    def __init__(
        ...
        self.anchor_parameters = (
            AnchorParameters.default
            if not self.detect_text
            else AnchorParameters(
                ratios=(0.25, 0.5, 1.0, 2.0), sizes=(16, 32, 64, 128, 256)
            )
        )
        self.anchors = anchors_for_shape(
    ...
    def load_annotations_group(self, group):
        ...
        for annotations in annotations_group:
            assert isinstance(
                annotations, dict
            ), "'load_annotations' should return a list of dictionaries, received: {}".format(
                type(annotations)
            )
            assert (
    ...
    def filter_annotations(self, image_group, annotations_group, group):
        ...
        for index, (image, annotations) in enumerate(
            ...
            if len(invalid_indices):
                warnings.warn(
                    "Image with id {} (shape {}) contains the following invalid boxes: {}.".format(
                        group[index],
                        image.shape,
    ...
    def clip_transformed_annotations(self, image_group, annotations_group, group):
        ...
        for index, (image, annotations) in enumerate(
            ...
            # x1
            annotations["bboxes"][:, 0] = np.clip(
                annotations["bboxes"][:, 0], 0, image_width - 2
            )
            # y1
            annotations["bboxes"][:, 1] = np.clip(
                annotations["bboxes"][:, 1], 0, image_height - 2
            )
            # x2
            annotations["bboxes"][:, 2] = np.clip(
                annotations["bboxes"][:, 2], 1, image_width - 1
            )
            # y2
            annotations["bboxes"][:, 3] = np.clip(
                annotations["bboxes"][:, 3], 1, image_height - 1
            )
            # test x2 < x1 | y2 < y1 | x1 < 0 | y1 < 0 | x2 <= 0 | y2 <= 0 | x2 >= image.shape[1] | y2 >= image.shape[0]
            ...
            if len(small_indices):
                for k in annotations_group[index].keys():
                    annotations_group[index][k] = np.delete(
                        annotations[k], small_indices, axis=0
                    )
                # import cv2
    ...
    def random_misc_group(self, image_group, annotations_group):
        ...
        for index in range(len(image_group)):
            ...
            image_group[index], annotations_group[index] = self.random_misc_group_entry(
                image_group[index], annotations_group[index]
            )
